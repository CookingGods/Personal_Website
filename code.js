...
const scriptURL =
  "https://script.google.com/macros/s/AKfycbyDY1I0FLbOtKlIgyYlkLDAZTngCzD_dVw1jsQUtDrKNHRi4DuJOTRQ4O4WIwQGaSZ8/exec";
...
// Modal behavior for TI Capstone project
document.addEventListener("DOMContentLoaded", function () {
  const openBtn = document.getElementById("view-ti-capstone");
  const modal = document.getElementById("modal-ti");
  const closeBtn = modal ? modal.querySelector(".modal-close") : null;
  const footerClose = document.getElementById("modal-close-btn");

  function openModal() {
    if (!modal) return;
    modal.classList.add("show");
    modal.setAttribute("aria-hidden", "false");
    // trap focus could be added here for accessibility
  }
